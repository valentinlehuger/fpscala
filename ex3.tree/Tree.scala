...
object Tree {
    ...
    def fold(root: Tree[A])(f: A => B)(g: (B, B) => B): B = root match {
        case Leaf(n) => f(n)
        case Branch(l, r) => g(fold(l)(f)(g), fold(r)(f)(g))
    }

    def size2[A](root: Tree[A]): Int = fold(root)(_ => 1)( (_, _) => 1 + _ + _)

    def maximum2(root: Tree[Int]): Int = fold(root)(a => a)(_ max _)

    def depth2[A](root: Tree[A]): Int =  fold(root)(_ => 0)( (_, _) => 1 + (_ max _))
